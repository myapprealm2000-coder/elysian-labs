...
// ─── Demo Layer Composition ──────────────────────────────────────────────────
// Used in RightPanel Layers tab when no real elements exist
...
export const DEMO_LAYER_COMPOSITIONS: DemoLayer[][] = [
  [
    {
      id: "dl-1",
      name: "Background",
...
// ─── Demo Project type (for dashboard preview) ────────────────────────────────
...
// Convert ISO date string to nanosecond bigint timestamp
function isoToBigintNs(iso: string): bigint {
  return BigInt(new Date(iso).getTime()) * 1_000_000n;
}

export const SAMPLE_PROJECTS: DemoProject[] = [
  {
    id: "demo-1",
...
// Gradient thumbnails by project type (for demo card visuals)
export const DEMO_PREVIEW_GRADIENTS: Record<TemplateType, string> = {
